...
main (void)
{
  ...
  char *log_entry_desc = "QWB Manual Group Sell";
  ...
  char *pay_from, *amount, *purpose, *file_date, *sellers;
  ...
  if (!purpose)
    purpose = "";
  sellers = cgi_s_get_param (params, params_len, "sellers");
  if (!sellers)
    sellers = "";
  ...
  { /* Append our log entries.  */
    char *cur_seller = sellers, *seller_end;
    do {
      bool last_line = false, cr_exist = false;
      seller_end = strchr (cur_seller, '\n');
      if (seller_end == NULL) {
	seller_end = cur_seller + strlen (cur_seller);
	last_line = true;
      } else
	*seller_end = '\0'; /* rewrite newline to null character */
      if (cur_seller == seller_end ||
	  !last_line && *cur_seller == '\r' && cur_seller + 1 == seller_end)
	...
	*(seller_end - 1) = '\0';
      ...
      fprintf (fp, "Pay To:%s\n", cur_seller);
      fprintf (fp, "Amount:%s\n", amount);
      fprintf (fp, "Purpose:Group Sell:%s\n", purpose);
    line_finished:
